...
module Huntress
  ...
  ##
  # Represents an organization in the Huntress system.
  #
  ...
  AlertData = Struct.new(:id, :created, :description, :severity, :category,
    ...
    #
    def create_endpoint
      Huntress::EndpointData.new(endpoint_id, category, endpoint_type)
    end
  end
  ...
  class ClientWrapper
    ...
    def initialize(client_id, client_secret, log = true)
      ...
      Huntress.configure do |config|
        ...
        config.logger = Logger.new(FileUtil.daily_file_name('huntress.log')) if log
      end
      @api = Huntress.client
      @api.login
    end
    ...
    def alerts(tenant = nil)
      @alerts ||= load_alerts
      if tenant
#        @alerts.values.select { |alert| tenant.eql?(alert.property('organization.id')) }
    ...
    def load_endpoints
      ...
      data = @api.agents
      data.each do |item|
        @endpoints[item.id] = EndpointData.new(item.id, item.platform, item.hostname, item.organization_id, '?', item.attributes)
      end
      @endpoints
    end
